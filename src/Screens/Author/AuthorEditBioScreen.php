...
class AuthorEditBioScreen extends BaseMachinimaScreen
{
    public function supports(array $update) : bool
    {
        ...
        if ('author' === $payload['domain'] && in_array($payload['action'], ['edit_bio', 'set_about'])) {
            return true;
        }

        $userId = $update['callback_query']['from']['id'] ?? $update['message']['from']['id'] ?? 0;
        $stateCheck = $this->userStateRepo->get($userId, 'set_author_about');
    ...
    public function handle(array $update) : void
    {
        ...
        if ('author' === $payload['domain'] && in_array($payload['action'], ['edit_bio', 'set_about'])) {
            ...
            $this->userStateRepo->set($userId, ['author_id' => $authorId], 'set_author_about');
        ...
        if (isset($update['message'])) {
            ...
            $state = $this->userStateRepo->get($userId, 'set_author_about');
            $this->userStateRepo->clear($userId, 'set_author_about');
